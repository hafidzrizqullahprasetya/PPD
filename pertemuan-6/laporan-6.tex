...
\begin{figure}[H]
\centering
\fbox{\includegraphics[width=0.72\textwidth]{images/gambar-6-2-1-supervised-vs-unsupervised.png}}
...
\begin{figure}[H]
\centering
\fbox{\includegraphics[width=0.75\textwidth]{images/gambar-6-2-2-customer-segmentation.png}}
...
\begin{figure}[H]
\centering
\fbox{\includegraphics[width=0.7\textwidth]{images/gambar-6-2-3-k-means-concept.png}}
...
\begin{figure}[H]
\centering
\fbox{\includegraphics[width=0.85\textwidth]{images/gambar-6-2-4-k-means-step-1.png}}
\caption{Tahap penentuan centroid awal dan penetapan titik terdekat \cite{tan2019}.}
\label{fig:kmeans-step1}
\end{figure}

\begin{figure}[H]
\centering
\fbox{\includegraphics[width=0.85\textwidth]{images/gambar-6-2-5-k-means-step-2.png}}
\caption{Tahap pembaruan centroid berdasarkan rata-rata anggota klaster \cite{tan2019}.}
\label{fig:kmeans-step2}
\end{figure}

\begin{figure}[H]
\centering
\fbox{\includegraphics[width=0.85\textwidth]{images/gambar-6-2-6-k-means-convergence.png}}
...
\begin{figure}[H]
\centering
\fbox{\includegraphics[width=0.85\textwidth]{images/gambar-6-2-7-k-means-outlier-problem.png}}
...
\begin{figure}[H]
\centering
\fbox{\includegraphics[width=0.85\textwidth]{images/gambar-6-2-7b-menentukan-k-elbow-silhouette.png}}
...
\begin{figure}[H]
\centering
\fbox{\includegraphics[width=0.8\textwidth]{images/gambar-6-2-8-k-medoids-vs-k-means.png}}
...
Ketahanan Pencilan & Peka terhadap pencilan & Tahan terhadap pencilan dan derau \\
Kompleksitas & Cepat, linier $O(t \cdot K \cdot n \cdot d)$ & Lebih berat, kuadratik $O(t \cdot K \cdot (n-K)^2)$ \\
\bottomrule
\end{tabular}
...
Sebagai simulasi numerik pada sepuluh titik data dua dimensi: $O_1(2,6)$, $O_2(3,4)$, $O_3(3,8)$, $O_4(4,7)$, $O_5(6,2)$, $O_6(6,4)$, $O_7(7,3)$, $O_8(7,4)$, $O_9(8,5)$, dan $O_{10}(7,6)$. Untuk $K = 2$ dengan medoid awal $(O_2, O_8)$, total jarak pada klaster 1 bernilai 11 dan klaster 2 bernilai 9, sehingga $E_{\text{lama}} = 20$, seperti tampak pada Gambar~\ref{fig:kmed-error}.

\begin{figure}[H]
\centering
\fbox{\includegraphics[width=0.85\textwidth]{images/gambar-6-2-10-k-medoids-pam-error-criterion.png}}
\caption{Perhitungan kriteria galat absolut $E$ pada pasangan medoid awal $(O_2, O_8)$ \cite{kaufman1990}.}
\label{fig:kmed-error}
\end{figure}

Ketika medoid $O_8$ dicoba ditukar dengan $O_7(7,3)$, total jarak klaster 1 tetap 11 dan klaster 2 menjadi 11, sehingga $E_{\text{baru}} = 22$. Nilai biaya swapping menjadi:
\begin{equation}
S = E_{\text{baru}} - E_{\text{lama}} = 22 - 20 = +2
\end{equation}
Karena $S > 0$, pertukaran tersebut ditolak dan $O_8$ tetap dipertahankan sebagai medoid, seperti ditunjukkan pada Gambar~\ref{fig:kmed-cost}.

\begin{figure}[H]
\centering
\fbox{\includegraphics[width=0.85\textwidth]{images/gambar-6-2-11-k-medoids-pam-cost-function.png}}
\caption{Evaluasi fungsi biaya pertukaran medoid bernilai positif sehingga pertukaran ditolak \cite{kaufman1990}.}
\label{fig:kmed-cost}
\end{figure}
...
\begin{figure}[H]
\centering
\fbox{\includegraphics[width=0.75\textwidth]{images/gambar-6-2-12-dbscan-density-clusters.png}}
...
\begin{figure}[H]
\centering
\fbox{\includegraphics[width=0.85\textwidth]{images/gambar-6-2-13-why-dbscan-comparison.png}}
...
\begin{itemize}
    ...
    \item Titik Inti (Core Point): Titik $p$ yang memiliki setidaknya $MinPts$ tetangga, yaitu $|N_\epsilon(p)| \ge MinPts$, seperti diilustrasikan pada Gambar~\ref{fig:dbscan-core}.
...
\begin{figure}[H]
\centering
\fbox{\includegraphics[width=0.85\textwidth]{images/gambar-6-2-14-dbscan-core-object.png}}
\caption{Ilustrasi titik inti (core object) dengan batas $\epsilon$ dan syarat $MinPts$ \cite{ester1996}.}
...
\end{enumerate}
Pembagian ketiga tipe titik ini ditunjukkan pada Gambar~\ref{fig:dbscan-types}.

\begin{figure}[H]
\centering
\fbox{\includegraphics[width=0.75\textwidth]{images/gambar-6-2-15-dbscan-point-types.png}}
\caption{Tiga tipe titik pada DBSCAN: core point, border point, dan noise point \cite{sander1998}.}
\label{fig:dbscan-types}
\end{figure}

Penentuan parameter $MinPts$ mengikuti aturan praktis: untuk data 2 dimensi digunakan $MinPts = 4$ \cite{ester1996}, sedangkan untuk dimensi $D > 2$ disarankan $MinPts = 2 \times D$ \cite{sander1998}. Nilai $\epsilon$ ditentukan memakai grafik jarak k-tetangga terurut (sorted k-distance graph) dengan $k = MinPts$. Nilai $\epsilon$ optimal diambil pada titik kelokan kurva, seperti tampak pada Gambar~\ref{fig:dbscan-kdist}.

\begin{figure}[H]
\centering
\fbox{\includegraphics[width=0.6\textwidth]{images/gambar-6-2-16-dbscan-k-dist-graph.png}}
...
\begin{figure}[H]
\centering
\fbox{\includegraphics[width=0.85\textwidth]{images/gambar-6-2-17-association-rule-transactions.png}}
...
Tiga metrik kualitas aturan asosiasi ditampilkan pada Gambar~\ref{fig:assoc-metrics}:
\begin{enumerate}
    \item \textbf{Support ($s$):} Fraksi transaksi dalam database yang memuat kombinasi $X$ dan $Y$ bersamaan:
    \begin{equation}
    s(X \rightarrow Y) = \frac{\sigma(X \cup Y)}{|T|}
    \end{equation}
    \item \textbf{Confidence ($c$):} Peluang bersyarat kemunculan $Y$ pada transaksi yang telah memuat $X$:
    \begin{equation}
    c(X \rightarrow Y) = \frac{\sigma(X \cup Y)}{\sigma(X)}
    \end{equation}
    \item \textbf{Lift:} Perbandingan kekuatan aturan terhadap asumsi bahwa kemunculan $X$ dan $Y$ saling bebas secara statistik:
    \begin{equation}
    Lift(X \rightarrow Y) = \frac{c(X \rightarrow Y)}{s(Y)} = \frac{s(X \cup Y)}{s(X) \cdot s(Y)}
    \end{equation}
    Jika $Lift = 1$, kejadian $X$ dan $Y$ independen. Jika $Lift > 1$, terdapat asosiasi positif di mana kemunculan $X$ meningkatkan kemungkinan kemunculan $Y$. Jika $Lift < 1$, hubungan bersifat substitutif atau saling meniadakan.
\end{enumerate}

\begin{figure}[H]
\centering
\fbox{\includegraphics[width=0.85\textwidth]{images/gambar-6-2-17b-association-rule-metrics.png}}
\caption{Metrik evaluasi aturan asosiasi: Support, Confidence, dan Lift \cite{tan2019}.}
\label{fig:assoc-metrics}
\end{figure}

Pada basis data berisi lima transaksi:
\begin{itemize}
    \item TID 1: $\{Bread, Milk\}$
    \item TID 2: $\{Bread, Vegetables, Fruits, Eggs\}$
    \item TID 3: $\{Milk, Vegetables, Fruits, Coke\}$
    \item TID 4: $\{Bread, Milk, Vegetables, Fruits\}$
    \item TID 5: $\{Bread, Milk, Vegetables, Coke\}$
\end{itemize}
Perhitungan untuk aturan $\{Milk, Vegetables\} \rightarrow \{Fruits\}$ menghasilkan $Support = 2/5 = 0{,}40$, $Confidence = 2/3 \approx 0{,}67$, dan $Lift = 0{,}40 / (0{,}60 \times 0{,}60) \approx 1{,}11$, seperti dirangkum pada Gambar~\ref{fig:assoc-calc}. Nilai Lift di atas 1 mengonfirmasi bahwa aturan ini memiliki hubungan asosiasi positif.

\begin{figure}[H]
\centering
\fbox{\includegraphics[width=0.85\textwidth]{images/gambar-6-2-17c-association-rule-example.png}}
\caption{Langkah perhitungan metrik Support, Confidence, dan Lift pada contoh data transaksi \cite{tan2019}.}
\label{fig:assoc-calc}
\end{figure}


\section{Algoritma Apriori}
Untuk basis data dengan $d$ item berbeda, ruang pencarian memuat $2^d - 1$ kemungkinan kombinasi itemset. Algoritma Apriori diperkenalkan oleh Rakesh Agrawal dan Ramakrishnan Srikant pada tahun 1994 untuk menekan ledakan kombinatorial ini \cite{agrawal1994}.

\begin{figure}[H]
\centering
\fbox{\includegraphics[width=0.75\textwidth]{images/gambar-6-2-18-apriori-principle.png}}
\caption{Prinsip Apriori: sifat penurunan monoton pada nilai support \cite{agrawal1994}.}
\label{fig:apriori-principle}
\end{figure}

Prinsip Apriori berpegang pada sifat anti-monoton nilai support:
\begin{quote}
Jika suatu itemset bersifat frequent, maka seluruh himpunan bagiannya (subset) pasti frequent. Sebaliknya, jika suatu itemset tidak frequent, maka seluruh himpunan yang memuatnya (superset) dipastikan tidak frequent.
\end{quote}
Prinsip ini ditunjukkan pada Gambar~\ref{fig:apriori-principle}.

Dengan prinsip tersebut, dilakukan pemangkasan kisi kandidat (lattice pruning). Jika itemset $\{A, B\}$ berada di bawah ambang batas minimum support, seluruh superset di atasnya seperti $\{A, B, C\}$ hingga $\{A, B, C, D, E\}$ langsung dipangkas tanpa perlu dihitung, seperti tampak pada Gambar~\ref{fig:apriori-lattice}.

\begin{figure}[H]
\centering
\fbox{\includegraphics[width=0.85\textwidth]{images/gambar-6-2-20-apriori-lattice-superset.png}}
\caption{Pemangkasan ruang pencarian superset yang tidak frequent pada kisi itemset \cite{tan2019}.}
...
\begin{figure}[H]
\centering
\fbox{\includegraphics[width=0.8\textwidth]{images/gambar-6-2-19-apriori-lattice-pruning.png}}
...
Sesuai Modul Pertemuan 6, alat dan bahan yang saya gunakan dalam praktikum ini meliputi:
...
\begin{figure}[H]
\centering
\customfbox{\includegraphics[width=0.7\textwidth]{images/placeholder.png}}
...
\begin{figure}[H]
\centering
\customfbox{\includegraphics[width=0.8\textwidth]{images/placeholder.png}}
...
\begin{figure}[H]
\centering
\customfbox{\includegraphics[width=0.85\textwidth]{images/placeholder.png}}
\caption{Tangkapan layar visualisasi kurva Elbow Method K-Means.}
...
\centering
\customfbox{\includegraphics[width=0.85\textwidth]{images/placeholder.png}}
\caption{Tangkapan layar plot konfirmasi titik siku K = 4 oleh KneeLocator.}
...
\centering
\customfbox{\includegraphics[width=0.85\textwidth]{images/placeholder.png}}
\caption{Tangkapan layar plot kurva Elbow K-Medoids.}
...
\centering
\customfbox{\includegraphics[width=0.85\textwidth]{images/placeholder.png}}
\caption{Tangkapan layar validasi titik siku K-Medoids pada K = 4.}
...
\centering
\customfbox{\includegraphics[width=0.9\textwidth]{images/placeholder.png}}
\caption{Tangkapan layar perhitungan Silhouette Score K-Means.}
...
\centering
\customfbox{\includegraphics[width=0.85\textwidth]{images/placeholder.png}}
\caption{Tangkapan layar plot grafik Silhouette Score K-Means.}
...
\centering
\customfbox{\includegraphics[width=0.9\textwidth]{images/placeholder.png}}
\caption{Tangkapan layar nilai Silhouette Score K-Medoids.}
...
\centering
\customfbox{\includegraphics[width=0.85\textwidth]{images/placeholder.png}}
\caption{Tangkapan layar plot kurva Silhouette Score K-Medoids.}
...
\centering
\customfbox{\includegraphics[width=0.85\textwidth]{images/placeholder.png}}
\caption{Tangkapan layar visualisasi 2D K-Means memakai Matplotlib.}
...
\centering
\customfbox{\includegraphics[width=0.85\textwidth]{images/placeholder.png}}
\caption{Tangkapan layar visualisasi 2D K-Means memakai Seaborn.}
...
plt.figure(figsize=(8, 6))
sns.scatterplot(x='PURCHASES', y='PAYMENTS', hue='cluster_labels_kmed', data=df_new, palette='Paired')
plt.legend(loc='lower right')
...
\centering
\customfbox{\includegraphics[width=0.85\textwidth]{images/placeholder.png}}
\caption{Tangkapan layar visualisasi sebaran klaster K-Medoids memakai Matplotlib.}
...
\centering
\customfbox{\includegraphics[width=0.85\textwidth]{images/placeholder.png}}
\caption{Tangkapan layar visualisasi sebaran klaster K-Medoids memakai Seaborn.}
...
\midrule
Cluster 0 & \$801{,}17 & \$503{,}25 & \$910{,}76 & \$388{,}15 & \$3.268{,}40 \\
Cluster 1 & \$2.198{,}42 & \$572{,}11 & \$1.332{,}45 & \$1.782{,}90 & \$4.512{,}30 \\
Cluster 2 & \$2.205{,}41 & \$4.250{,}80 & \$4.120{,}15 & \$452{,}10 & \$7.750{,}20 \\
Cluster 3 & \$4.890{,}62 & \$610{,}45 & \$3.620{,}80 & \$4.520{,}35 & \$8.920{,}50 \\
\bottomrule
\end{tabular}
...
\begin{enumerate}
    \item \textbf{Cluster 0 (Nasabah Inaktif / Saldo Rendah):}
    \begin{itemize}
        \item Karakteristik: Nilai rata-rata pembelian (\$503), pembayaran tagihan (\$910), dan saldo utang (\$801) berada pada tingkat terendah. Nasabah jarang memakai kartu kredit.
        \item Rekomendasi Bisnis: Penawaran program bebas iuran tahunan (annual fee waiver), promo diskon transaksi pertama pada merchant rekanan, dan notifikasi aktivasi berkala untuk mendorong pemakaian kartu.
    \end{itemize}
    \item \textbf{Cluster 1 (Nasabah Transaksi Rutin / Pengeluaran Moderat):}
    \begin{itemize}
        \item Karakteristik: Menunjukkan nilai pembelian, pembayaran, dan saldo pada rentang sedang. Nasabah memakai kartu secara teratur untuk belanja bulanan dan melunasi tagihan secara berimbang.
        \item Rekomendasi Bisnis: Pemberian poin loyalitas (reward points), fasilitas cicilan bunga 0\% pada merchant elektronik atau supermarket, dan penawaran cashback kategori kebutuhan pokok.
    \end{itemize}
    \item \textbf{Cluster 2 (Nasabah Pembeli Aktif / Pengeluaran Tinggi):}
    \begin{itemize}
        \item Karakteristik: Memiliki rata-rata pembelian tertinggi (\$4.250) dan pembayaran tertinggi (\$4.120), dengan limit kredit besar (\$7.750). Nasabah memiliki daya beli tinggi dan melunasi tagihan dengan tertib.
        \item Rekomendasi Bisnis: Peningkatan limit kredit otomatis, undangan peningkatan ke tier kartu premium (Platinum atau World Elite), serta fasilitas prioritas bandara dan perlindungan asuransi perjalanan.
    \end{itemize}
    \item \textbf{Cluster 3 (Nasabah Saldo Tinggi / Pengguna Dana Tunai):}
    \begin{itemize}
        \item Karakteristik: Memiliki saldo utang tertinggi (\$4.890) dan penarikan tunai tertinggi (\$4.520). Nasabah mempertahankan saldo utang bergulir dan rutin memakai penarikan tunai lewat ATM.
        \item Rekomendasi Bisnis: Segmen ini menghasilkan marjin bunga tinggi bagi perbankan, tetapi memiliki risiko kredit lebih besar. Bank dapat menawarkan restrukturisasi saldo menjadi cicilan tetap dengan suku bunga kompetitif untuk menekan potensi gagal bayar (default).
    \end{itemize}
\end{enumerate}
...
\centering
\scriptsize
\begin{tabular}{p{0.5cm} p{4.6cm} p{1.6cm} p{6.8cm}}
\toprule
\textbf{No} & \textbf{Nama Fitur} & \textbf{Tipe Data} & \textbf{Deskripsi dan Makna Operasional} \\
\midrule
1 & \texttt{CUST\_ID} & String & Nomor identitas unik pemegang kartu kredit. Tidak digunakan dalam pemodelan karena bersifat kategorikal murni. \\
2 & \texttt{BALANCE} & Float & Jumlah saldo utang yang masih tersisa pada rekening kartu kredit nasabah. \\
3 & \texttt{BALANCE\_FREQUENCY} & Float & Rasio keteraturan saldo diperbarui, dengan skala 0 (tidak pernah diperbarui) hingga 1 (selalu diperbarui berkala). \\
4 & \texttt{PURCHASES} & Float & Total akumulasi nominal pembelanjaan yang dilakukan nasabah selama 6 bulan. \\
5 & \texttt{ONEOFF\_PURCHASES} & Float & Total akumulasi nominal transaksi belanja yang dibayar tunai sekaligus tanpa cicilan. \\
6 & \texttt{INSTALLMENTS\_PURCHASES} & Float & Total akumulasi nominal transaksi belanja yang dibayar melalui skema cicilan bertahap. \\
7 & \texttt{CASH\_ADVANCE} & Float & Total nominal dana tunai yang ditarik nasabah di muka melalui mesin ATM. \\
8 & \texttt{PURCHASES\_FREQUENCY} & Float & Frekuensi transaksi pembelian oleh nasabah, bernilai 0 sampai 1. \\
9 & \texttt{ONEOFF\_PURCHASES\_FREQUENCY} & Float & Frekuensi transaksi belanja sekaligus tanpa cicilan (rasio 0 sampai 1). \\
10 & \texttt{PURCHASES\_INSTALLMENTS\_}\newline\texttt{FREQUENCY} & Float & Frekuensi transaksi belanja cicilan bertahap (rasio 0 sampai 1). \\
11 & \texttt{CASH\_ADVANCE\_FREQUENCY} & Float & Frekuensi transaksi penarikan uang tunai di muka (rasio 0 sampai 1). \\
12 & \texttt{CASH\_ADVANCE\_TRX} & Integer & Total frekuensi hitungan transaksi penarikan dana tunai yang dilakukan nasabah. \\
13 & \texttt{PURCHASES\_TRX} & Integer & Total hitungan transaksi pembelian yang dilakukan nasabah. \\
14 & \texttt{CREDIT\_LIMIT} & Float & Batas pagu kredit maksimum yang diberikan bank kepada nasabah. \\
15 & \texttt{PAYMENTS} & Float & Total nominal pembayaran tagihan yang telah disetorkan oleh nasabah. \\
16 & \texttt{MINIMUM\_PAYMENTS} & Float & Total nominal pembayaran minimum yang disetorkan nasabah untuk menjaga kartu tetap aktif. \\
17 & \texttt{PRC\_FULL\_PAYMENT} & Float & Persentase tagihan kartu kredit yang dilunasi penuh (100\%) oleh nasabah setiap bulan. \\
18 & \texttt{TENURE} & Integer & Masa aktif kepemilikan kartu kredit oleh nasabah (dalam satuan bulan, rentang 6 sampai 12). \\
\bottomrule
\end{tabular}
...
for k in range(1, 11):
    ...
    if k >= 2:
        silhouette_air_kmeans.append(silhouette_score(X_air_scaled, km.labels_))

kneedle_air = KneeLocator(range(1, 11), inertia_air_kmeans, curve='convex', direction='decreasing')
...
\centering
\customfbox{\includegraphics[width=0.85\textwidth]{images/placeholder.png}}
\caption{Tangkapan layar kurva penurunan inertia data penerbangan.}
...
\centering
\customfbox{\includegraphics[width=0.85\textwidth]{images/placeholder.png}}
\caption{Tangkapan layar kurva Silhouette Score data penerbangan.}
...
\centering
\customfbox{\includegraphics[width=0.85\textwidth]{images/placeholder.png}}
\caption{Tangkapan layar sebaran klaster 2D data penerbangan K-Means.}
...
\centering
\customfbox{\includegraphics[width=0.85\textwidth]{images/placeholder.png}}
\caption{Tangkapan layar sebaran klaster 2D data penerbangan K-Medoids.}
...
\subsubsection{Analisis Ketahanan terhadap Outlier (K-Means vs K-Medoids)}
...
\begin{enumerate}
    \item Unsupervised learning memodelkan struktur keteraturan pada data tanpa label target ($y$). Berbeda dengan supervised learning yang melatih fungsi prediksi berdasarkan acuan kebenaran (ground truth), unsupervised learning berfokus menemukan kelompok alami (clustering) dan pola asosiasi (association rules).
    \item Algoritma K-Means mempartisi data secara efisien dengan kompleksitas waktu berorde linier $O(t \cdot K \cdot n \cdot d)$. Namun, K-Means sensitif terhadap nilai pencilan karena titik pusatnya dihitung berdasarkan rata-rata aritmetika (mean vector) yang mudah bergeser oleh nilai ekstrem.
    \item Algoritma K-Medoids (PAM) memperbaiki kelemahan K-Means dengan memilih objek data riil aktual dari dataset sebagai pusat klaster (medoid) dan meminimalkan jumlah dissimilaritas absolut. Mekanisme evaluasi fungsi biaya swapping ($S = E_{\text{baru}} - E_{\text{lama}}$) memastikan titik pencilan tidak merusak stabilitas pusat klaster.
    \item Algoritma DBSCAN membentuk klaster berbasis kerapatan spasial melalui parameter radius $\epsilon$ dan $MinPts$. DBSCAN mampu memisahkan titik derau (noise) secara otomatis dan mengenali struktur klaster dengan bentuk non-sferis seperti cincin konsentris yang gagal dipisahkan oleh K-Means.
    \item Pada evaluasi klaster Credit Card Dataset, Elbow Method dan modul KneeLocator mengonfirmasi jumlah klaster optimal pada $K = 4$. Analisis profil klaster menghasilkan empat segmen nasabah: Cluster 0 (Nasabah Inaktif/Low Engagement), Cluster 1 (Nasabah Transaksi Rutin/Moderate Spenders), Cluster 2 (Nasabah Belanja Tinggi/VIP Prime), dan Cluster 3 (Nasabah Saldo dan Dana Tunai Tinggi/Revolvers).
    \item Eksperimen pada dataset Air Traffic Passenger Statistics membuktikan bahwa K-Medoids memiliki ketahanan yang lebih baik terhadap data pencilan operasional bandara (seperti dominasi maskapai hub United Airlines) dibandingkan K-Means yang titik centroidnya terdistorsi oleh nilai ekstrim.
    \item Pada Association Rule Mining, aturan $X \rightarrow Y$ mencerminkan kemunculan bersama (co-occurrence) dan bukan kausalitas sebab-akibat. Sifat anti-monoton pada prinsip Apriori memungkinkan pemangkasan ruang pencarian (lattice pruning) secara efisien dengan mengabaikan seluruh superset dari itemset yang terbukti tidak frequent.
\end{enumerate}
